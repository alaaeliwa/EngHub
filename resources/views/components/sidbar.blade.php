            ...
            <nav class="sidebar-nav">
                <div class="nav-group">
                    <a href="{{ route('dashboard') }}"
                        ...
                        <span>{{ __('messages.sidebar_dashboard') ?? 'Dashboard' }}</span>
                    </a>
                    <a href="{{ route('courses') }}"
                        class="nav-item {{ request()->routeIs('courses') ? 'active' : '' }}">
                        <i class="fa-solid fa-graduation-cap"></i>
                        <span>{{ __('messages.sidebar_courses') ?? 'Courses' }}</span>
                    </a>
                    <a href="{{ route('upload') }}" class="nav-item upload-nav">
                        <i class="fa-solid fa-cloud-arrow-up"></i>
                        <span>{{ __('messages.sidebar_uploads') ?? 'Uploads' }}</span>
                    </a>
                    <a href="{{ route('workshops') }}" class="nav-item">
                        <i class="fa-solid fa-laptop-code"></i>
                        <span>{{ __('messages.sidebar_workshops') ?? 'Workshops' }}</span>
                    </a>
                </div>
            ...
            <div class="sidebar-footer">
                ...
                <form method="POST" action="{{ route('logout') }}" style="margin: 0; padding: 0;">
                    ...
                    <button type="submit" class="btn btn-block"
                        ...
                        <span>{{ __('messages.sidebar_logout') ?? 'Logout' }}</span>
                    </button>
                </form>
            </div>
        </aside>
